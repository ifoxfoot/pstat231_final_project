...
library(VIM) #for analyzing missing data
library(missMDA) #f0r imputing missing data
library(ggfortify)
...
#select weather cols     
weather <- fire_modeling_df %>% 
  select(temp_pre_30:prec_cont)

#select cols not in weather
notweather <- fire_modeling_df %>% 
  select(-colnames(weather)) 

#Imputation before PCA
fire_imputed <- imputePCA(weather, ncp=4)

#bind imputed data to fire data
fire_complete <- cbind(notweather, fire_imputed$completeObs)

#one-hot code data
dummy <- dummyVars(" ~ .", data = fire_complete)
fire_one_hot <- data.frame(predict(dummy, newdata = fire_complete)) %>% 
...
weather_PCA <- weather_imp$completeObs %>% 
  scale(center = T, scale = T) %>% #first scale and center the data
  prcomp() #do PCA

#Make a PCA bi-plot
autoplot(weather_PCA, 
         data = weather,
         loadings = TRUE, 
         loadings.label = TRUE,
         loadings.label.hjust = 1.2) +
  theme_classic() +
  labs(caption = "Principle Component Analysis Bi-plot of Weather Data")
```
