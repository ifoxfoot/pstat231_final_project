...
```{r}
lm_whole = lm(fire_size ~ PC1 + PC2 + PC3 + PC4 + year_diff, data = fire_train_complete)
summary(lm_whole)

lm_sw = lm(fire_size ~ PC1 + PC2 + PC3 + PC4 + year_diff, data = fire_train_complete[which(fire_train_complete$region == "SouthWest"),])
summary(lm_sw)

lm_se = lm(fire_size ~ PC1 + PC2 + PC3 + PC4 + year_diff, data = fire_train_complete[which(fire_train_complete$region == "SouthEast"),])
summary(lm_se)

lm_me = lm(fire_size ~ PC1 + PC2 + PC3 + PC4 + year_diff, data = fire_train_complete[which(fire_train_complete$region == "MidWest"),])
summary(lm_me)

lm_w = lm(fire_size ~ PC1 + PC2 + PC3 + PC4 + year_diff, data = fire_train_complete[which(fire_train_complete$region == "West"),])
summary(lm_w)

lm_ne = lm(fire_size ~ PC1 + PC2 + PC3 + PC4 + year_diff, data = fire_train_complete[which(fire_train_complete$region == "NorthEast"),])
summary(lm_ne)
```
...
```{r}
#one-hot code data (not working)
# dummy_train <- dummyVars(" ~ .", data = fire_train_complete)
# fire_train_dummy <- data.frame(predict(dummy_train, newdata = fire_train_complete)) %>% 
